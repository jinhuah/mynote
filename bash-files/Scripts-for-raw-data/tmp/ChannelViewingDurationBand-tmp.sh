...
# Verify data correctness from raw data - a channel watched by devices
...
# Use devices array to store device id, maximan $dn number of devices.
dn=200
...
do 
	...
	do
		...
		then 
			...
			case 1 in
				$(($duration < 10 && $duration != 0 && ${durationBandTmp[0]} == 0 )) )	# 0-10 seconds
...
# echo "-----"
	...
	# Count devices in viewing duration band
	for d in ${durationBandTmp[@]};
	do
		case 1 in
			$(($d < 10 && $d != 0 )) )	# 0-10 seconds
