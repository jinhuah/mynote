...
do 
	...
	do
		...
		# If it reaches the last, a date is given and the start time is not empty, then get channel stop time from the following day.
		if [[ $line == $(tail -1 $TESTTMP2) && $( echo $DATE |  awk -F'-' '{print NF}') == 3 && $dStart != "" && $dStop == "" ]]
		then 
			echo -e "\ndevices$a: $i"
			echo "Start time: $dStart - $(date -ud @$dStart), Stop time: $dStop ---? "
			...
			echo -ne "New stop time: $dStop $(date -ud @$dStop), New start time: $dStartTmp $(date -ud @$dStartTmp), End of day: $END_TIME $(date -ud @$END_TIME)"
			[[ $dStop < $dStartTmp ]] && [[ $dStop > $END_TIME ]] || dStop=""
			echo "last line of stop time: $dStop "
		fi
